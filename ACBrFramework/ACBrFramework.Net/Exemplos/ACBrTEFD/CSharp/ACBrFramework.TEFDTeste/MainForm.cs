...
namespace ACBrFramework.TEFDTeste
{
    public partial class MainForm : Form
    {
        ...
        private void inicializarECFTEF()
        {
            ...
            chkAutoAtivarGP.Checked = acBrTEFD1.AutoAtivar;
            chkMultiplosCartoes.Checked = acBrTEFD1.MultiplosCartoes;
            chkMulti.Checked = acBrTEFD1.MultiplosCartoes;
            chkAutoEfetuarPagamento.Checked = acBrTEFD1.AutoEfetuarPagamento;
            chkAutoFinalizarCupom.Checked = acBrTEFD1.AutoFinalizarCupom;
            chkCHQGerencial.Checked = acBrTEFD1.CHQEmGerencial;
        ...
        private void analisaTEF()
        {
            ...
            if (acBrTEFD1.TEFCliSiTef.Habilitado)
            {
                pctSitef.Image = imgGreen;
                pctSitef.Refresh();
            }
            else
            {
                pctSitef.Image = imgRed;
                pctSitef.Refresh();
            }
        ...
        private void ReducaoZ()
        {
            ...
            {
                if (
                    MessageBox.Show(@"A Redução Z pode Bloquear o seu ECF até a 12:00pm.\nContinuar assim mesmo ?",
                        "TEF", MessageBoxButtons.YesNo, MessageBoxIcon.Question).Equals(DialogResult.Yes))
                {
                    acBrECF1.ReducaoZ();
        ...
        private void Pagamento()
        {
            try
            {
                ...
                if (
                    InputBox.Show(string.Format("Pagamento de {0:c} Digite o Cod.Forma Pagamento", valor),
                        "Indice da Forma de Pagamento", ref indice).Equals(DialogResult.OK))
                {
        ...
        private void AdicionarPagamento()
        {
            try
            {
                ...
                if (
                    InputBox.Show("Pagamento", "Digite o indice da forma de pagamento", ref indice)
        ...
        private void tef_OnInfoECF(object sender, InfoECFEventArgs e)
        {
            ...
            catch (Exception)
            {
                ...
                if (!acBrTEFD1.Inicializado() & //Está na inicialização ?
                    e.Operacao == InfoECF.EstadoECF & //Quer Saber o estado do ECF ? (mas se chegou aqui é pq o ECF já está com problemas)
                    (acBrTEFD1.GPAtual == TefTipo.CliSiTef | acBrTEFD1.GPAtual == TefTipo.VeSPague))
                {
                    ...
                    */
                    ...
                    e.RetornoECF = RetornoECF.CDCouRelatorioGerencial;    // Executará ConfirmarESolicitarImpressaoTransacoesPendentes;
                    return;
                }
                throw;
            }
		...
		private void TEFCliSiTef_OnObtemCampo(object sender, TEFCliSiTefObtemCampoEventArgs e)
		{
			using (InputForm Input = new InputForm())
			{
				Input.Initialize(e);
			    Input.ShowDialog();
			    e.Digitado = Input.DialogResult == DialogResult.OK;
                e.VoltarMenu = Input.DialogResult == DialogResult.Retry;
                if (e.Digitado)
