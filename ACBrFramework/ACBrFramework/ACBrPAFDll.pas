...
type
  TRegistroJ1Rec = record
    QTD_J2: integer;
    CNPJ: array[0..14] of char;
    DATA_EMISSAO: double;
    SUBTOTAL: double;
    DESC_SUBTOTAL: double;
    INDICADOR_DESC: array[0..1] of char;
    ACRES_SUBTOTAL: double;
    INDICADOR_ACRES: array[0..1] of char;
    VALOR_LIQUIDO: double;
    INDICADOR_CANC: array[0..1] of char;
    VAL_CANC_ACRES: double;
    ORDEM_APLIC_DES_ACRES: array[0..1] of char;
    NOME_CLIENTE: array[0..40] of char;
    CPFCNPJ_CLIENTE: array[0..14] of char;
    NUMERO_NOTA: array[0..10] of char;
    SERIE_NOTA: array[0..3] of char;
    CHAVE_NF: array[0..44] of char;
    TIPO_DOC: array[0..1] of char;
    RegistroValido: boolean;
  end;

type
  TRegistroJ2Rec = record
    CNPJ: array[0..14] of char;
    DATA_EMISSAO: double;
    NUMERO_ITEM: array[0..3] of char;
    CODIGO_PRODUTO: array[0..14] of char;
    DESCRICAO: array[0..100] of char;
    QUANTIDADE: double;
    UNIDADE: array[0..3] of char;
    VALOR_UNITARIO: double;
    DESCONTO_ITEM: double;
    ACRESCIMO_ITEM: double;
    VALOR_LIQUIDO: double;
    TOTALIZADOR_PARCIAL: array[0..7] of char;
    CASAS_DECIMAIS_QTDE: array[0..1] of char;
    CASAS_DECIMAIS_VAL_UNIT: array[0..1] of char;
    NUMERO_NOTA: array[0..10] of char;
    SERIE_NOTA: array[0..3] of char;
    CHAVE_NF: array[0..44] of char;
    TIPO_DOC: array[0..1] of char;
...
function PAF_Preenche_A(const pafHandle: PPAFHandle;
  var RegistroA2Rec: array of TRegistroA2Rec; const CountA2: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_B(const pafHandle: PPAFHandle;
  const RegistroB1Rec: TRegistroHD1Rec; var RegistroB2Rec: array of TRegistroB2Rec;
  const CountB2: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_C(const pafHandle: PPAFHandle;
  const RegistroC1Rec: TRegistroHD1Rec; var RegistroC2Rec: array of TRegistroC2Rec;
  const CountC2: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_D(const pafHandle: PPAFHandle;
  const RegistroD1Rec: TRegistroHD1Rec; var RegistroD2Rec: array of TRegistroD2Rec;
  const CountD2: integer; var RegistroD3Rec: array of TRegistroD3Rec;
  var RegistroD4Rec: array of TRegistroD4Rec; const CountD4: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_E(const pafHandle: PPAFHandle;
  const RegistroE1Rec: TRegistroHD2Rec; var RegistroE2Rec: array of TRegistroE2Rec;
  const RegistroE3Rec: TRegistroE3Rec; const CountE2: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_H(const pafHandle: PPAFHandle;
  const RegistroH1Rec: TRegistroHD2Rec; var RegistroH2Rec: array of TRegistroH2Rec;
  const CountH2: integer): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_J(const pafHandle: PPAFHandle;
   var RegistroJ1Rec: array of TRegistroJ1Rec;
   const CountJ1: integer; var RegistroJ2Rec: array of TRegistroJ2Rec): integer;
  {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_P(const pafHandle: PPAFHandle;
  const RegistroP1Rec: TRegistroHD1Rec; var RegistroP2Rec: array of TRegistroP2Rec;
...
function PAF_Preenche_N(const pafHandle: PPAFHandle;
  ...
  var RegistroN3Rec: array of TRegistroN3Rec; const QTD_N3: integer): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_R(const pafHandle: PPAFHandle; const CountR1: integer;
  var RegistroR1Rec: array of TRegistroR1Rec;
  var RegistroR2Rec: array of TRegistroR2Rec;
  var RegistroR3Rec: array of TRegistroR3Rec;
  var RegistroR4Rec: array of TRegistroR4Rec;
  var RegistroR5Rec: array of TRegistroR5Rec;
  var RegistroR6Rec: array of TRegistroR6Rec;
  var RegistroR7Rec: array of TRegistroR7Rec): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_Preenche_S(const pafHandle: PPAFHandle;
  const CountS2: integer;
  var RegistroS2Rec: array of TRegistroS2Rec;
  var RegistroS3Rec: array of TRegistroS3Rec): integer;
 {$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

 function PAF_Preenche_T(const pafHandle: PPAFHandle;
   const RegistroT1Rec : TRegistroHD1Rec; var RegistroT2Rec : array of TRegistroT2Rec;
...
function PAF_Preenche_TITP(const pafHandle: PPAFHandle;
  var RegistroMercadoriasRec: array of TRegistroMercadoriaRec;
  const Count: integer; var RegistroInsumosRec: array of TRegistroInsumoRec;
  const Titulo: PChar; const Data: double): integer;{$IFDEF STDCALL} stdcall;
 {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

{%endregion}

function PAF_SaveFileTXT_B(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_C(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_D(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_E(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_H(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall;
 {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

 function PAF_SaveFileTXT_J(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
 {$IFDEF STDCALL} stdcall;
  {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_N(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_P(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_R(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;

function PAF_SaveFileTXT_T(const pafHandle: PPAFHandle; const Arquivo: PChar): integer;
{$IFDEF STDCALL} stdcall; {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;
...
begin
  ...
  try
    ...
    pafHandle^.EventHandlers := TEventHandlersPAF.Create();
...
begin
  ...
  try
    IndexItem := 0;
    pafHandle^.PAF.PAF_J.RegistroJ1.Clear;

    for i := 0 to CountJ1 - 1 do
    begin
      with pafHandle^.PAF.PAF_J.RegistroJ1.New do
      begin
        CNPJ := RegistroJ1Rec[i].CNPJ;
        DATA_EMISSAO := RegistroJ1Rec[i].DATA_EMISSAO;
        SUBTOTAL := RegistroJ1Rec[i].SUBTOTAL;
        DESC_SUBTOTAL := RegistroJ1Rec[i].DESC_SUBTOTAL;
        INDICADOR_DESC := RegistroJ1Rec[i].INDICADOR_DESC;
        ACRES_SUBTOTAL := RegistroJ1Rec[i].ACRES_SUBTOTAL;
        INDICADOR_ACRES := RegistroJ1Rec[i].INDICADOR_ACRES;
        VALOR_LIQUIDO := RegistroJ1Rec[i].VALOR_LIQUIDO;
        INDICADOR_CANC := RegistroJ1Rec[i].INDICADOR_CANC;
        VAL_CANC_ACRES := RegistroJ1Rec[i].VAL_CANC_ACRES;
        ORDEM_APLIC_DES_ACRES := RegistroJ1Rec[i].ORDEM_APLIC_DES_ACRES;
        NOME_CLIENTE := RegistroJ1Rec[i].NOME_CLIENTE;
        CPFCNPJ_CLIENTE := RegistroJ1Rec[i].CPFCNPJ_CLIENTE;
        NUMERO_NOTA := RegistroJ1Rec[i].NUMERO_NOTA;
        SERIE_NOTA := RegistroJ1Rec[i].SERIE_NOTA;
        CHAVE_NF := RegistroJ1Rec[i].CHAVE_NF;
        TIPO_DOC := RegistroJ1Rec[i].TIPO_DOC;
        RegistroValido := RegistroJ1Rec[i].RegistroValido;

        for D := 0 to RegistroJ1Rec[i].QTD_J2 - 1 do
        begin
          // adicionar os itens da nota fiscal
          with RegistroJ2.New do
          begin
            CNPJ := RegistroJ2Rec[IndexItem].CNPJ;
            DATA_EMISSAO := RegistroJ2Rec[IndexItem].DATA_EMISSAO;
            NUMERO_ITEM := RegistroJ2Rec[IndexItem].NUMERO_ITEM;
            CODIGO_PRODUTO := RegistroJ2Rec[IndexItem].CODIGO_PRODUTO;
            DESCRICAO := RegistroJ2Rec[IndexItem].DESCRICAO;
            QUANTIDADE := RegistroJ2Rec[IndexItem].QUANTIDADE;
            UNIDADE := RegistroJ2Rec[IndexItem].UNIDADE;
            VALOR_UNITARIO := RegistroJ2Rec[IndexItem].VALOR_UNITARIO;
            DESCONTO_ITEM := RegistroJ2Rec[IndexItem].DESCONTO_ITEM;
            ACRESCIMO_ITEM := RegistroJ2Rec[IndexItem].ACRESCIMO_ITEM;
            VALOR_LIQUIDO := RegistroJ2Rec[IndexItem].VALOR_LIQUIDO;
            TOTALIZADOR_PARCIAL := RegistroJ2Rec[IndexItem].TOTALIZADOR_PARCIAL;
            CASAS_DECIMAIS_QTDE := RegistroJ2Rec[IndexItem].CASAS_DECIMAIS_QTDE;
            CASAS_DECIMAIS_VAL_UNIT := RegistroJ2Rec[IndexItem].CASAS_DECIMAIS_VAL_UNIT;
            NUMERO_NOTA := RegistroJ2Rec[IndexItem].NUMERO_NOTA;
            SERIE_NOTA := RegistroJ2Rec[IndexItem].SERIE_NOTA;
            CHAVE_NF := RegistroJ2Rec[IndexItem].CHAVE_NF;
            TIPO_DOC := RegistroJ2Rec[IndexItem].TIPO_DOC;
            RegistroValido := RegistroJ2Rec[IndexItem].RegistroValido;
          end;
          Inc(IndexItem);
        end;
      end;
    ...
    begin
      pafHandle^.UltimoErro := Exception.Message;
      pafHandle^.PAF.PAF_J.LimpaRegistros;
      Result := -1;
    end
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        CEST := RegistroP2Rec[i].CEST;
        NCM := RegistroP2Rec[i].NCM;
...
function PAF_Preenche_N(const pafHandle: PPAFHandle;
  const RegistroN1Rec : TRegistroHD1Rec; const RegistroN2Rec : TRegistroN2Rec;
  var RegistroN3Rec : array of TRegistroN3Rec; const QTD_N3 : Integer)
...
function PAF_SaveFileTXT_J(const pafHandle: PPAFHandle;
  const Arquivo: PChar): integer;{$IFDEF STDCALL} stdcall;
 {$ENDIF} {$IFDEF CDECL} cdecl; {$ENDIF} export;
begin
  if (pafHandle = nil) then
