 ...
 * 
 * @author Jose Mauro
 * @version Criado em: 10/12/2013 14:10:14, revisao: $Id: ACBrPAF.java 14948 2018-04-05 04:10:35Z rfteno $
 */
public class ACBrPAF extends ACBrClass {
    ...
    /**
     * Registros {@see ACBrPAF_J}.
     */
    private ACBrPAF_J paf_J = new ACBrPAF_J();
    /**
    ...
    /**
     * Preenche os valores do PAF_J.
     * 
     * @throws ACBrException 
     */
    private void preenche_J() throws ACBrException {
        // Criando as variaveis para J1, J2.
        ACBrPAFInterop.RegistroJ1Rec[] lRegistrosJ1 = criaVetorJNA(ACBrPAFInterop.RegistroJ1Rec.class, paf_J.getRegistrosJ1().size());
        ACBrPAFInterop.RegistroJ2Rec[] lRegistrosJ2 = criaVetorJNA(ACBrPAFInterop.RegistroJ2Rec.class, paf_J.countJ2());
        
        int iDxJ1 = 0, iDxJ2 = 0;
        for (ACBrPAFRegistroJ1 lRegistroJ1 : paf_J.getRegistrosJ1()) {
            // Populando o registro J1
            lRegistrosJ1[iDxJ1].QTD_J2 = lRegistroJ1.getRegistrosJ2().size();
            lRegistrosJ1[iDxJ1].CNPJ = toByte(lRegistroJ1.getCnpj(), 21);
            lRegistrosJ1[iDxJ1].DATA_EMISSAO = OleDate.toOADate(lRegistroJ1.getDataEmissao());
            lRegistrosJ1[iDxJ1].SUBTOTAL = lRegistroJ1.getSubTotal();
            lRegistrosJ1[iDxJ1].DESC_SUBTOTAL = lRegistroJ1.getDescSubTotal();
            lRegistrosJ1[iDxJ1].INDICADOR_DESC = toByte(lRegistroJ1.getIndicadorDesc(), 2);
            lRegistrosJ1[iDxJ1].ACRES_SUBTOTAL = lRegistroJ1.getAcresSubTotal();
            lRegistrosJ1[iDxJ1].INDICADOR_ACRES = toByte(lRegistroJ1.getIndicadorAcres(), 2);
            lRegistrosJ1[iDxJ1].VALOR_LIQUIDO = lRegistroJ1.getValorLiquido();
            lRegistrosJ1[iDxJ1].INDICADOR_CANC = toByte(lRegistroJ1.getIndicadorCanc(), 2);
            lRegistrosJ1[iDxJ1].VAL_CANC_ACRES = lRegistroJ1.getValCancAcres();
            lRegistrosJ1[iDxJ1].ORDEM_APLIC_DES_ACRES = toByte(lRegistroJ1.getOrdemAplicDesAcres(), 2);
            lRegistrosJ1[iDxJ1].NOME_CLIENTE = toByte(lRegistroJ1.getNomeCliente(), 41);
            lRegistrosJ1[iDxJ1].CPFCNPJ_CLIENTE = toByte(lRegistroJ1.getCpfCnpjCliente(), 15);
            lRegistrosJ1[iDxJ1].NUMERO_NOTA = toByte(lRegistroJ1.getNumeroNota(), 11);
            lRegistrosJ1[iDxJ1].SERIE_NOTA = toByte(lRegistroJ1.getSerieNota(), 4);
            lRegistrosJ1[iDxJ1].CHAVE_NF = toByte(lRegistroJ1.getChaveNf(), 45);
            lRegistrosJ1[iDxJ1].TIPO_DOC = toByte(lRegistroJ1.getTipoDoc(), 2);
            lRegistrosJ1[iDxJ1].RegistroValido = converteBooleanToByte(lRegistroJ1.isRegistroValido());
            
            // Populando o registro J2
            for (ACBrPAFRegistroJ2 lRegistroJ2 : lRegistroJ1.getRegistrosJ2()) {
                lRegistrosJ2[iDxJ2].CNPJ = toByte(lRegistroJ2.getCnpj(), 21);
                lRegistrosJ2[iDxJ2].DATA_EMISSAO = OleDate.toOADate(lRegistroJ2.getDataEmissao());
                lRegistrosJ2[iDxJ2].NUMERO_ITEM  = toByte(lRegistroJ2.getNumeroItem(), 4);
                lRegistrosJ2[iDxJ2].CODIGO_PRODUTO = toByte(lRegistroJ2.getCodigoProduto(), 15);
                lRegistrosJ2[iDxJ2].DESCRICAO = toByte(lRegistroJ2.getDescricao(), 101);
                lRegistrosJ2[iDxJ2].QUANTIDADE = lRegistroJ2.getQuantidade();
                lRegistrosJ2[iDxJ2].UNIDADE = toByte(lRegistroJ2.getUnidade(), 4);
                lRegistrosJ2[iDxJ2].VALOR_UNITARIO = lRegistroJ2.getValorUnitario();
                lRegistrosJ2[iDxJ2].DESCONTO_ITEM = lRegistroJ2.getDescontoItem();
                lRegistrosJ2[iDxJ2].ACRESCIMO_ITEM = lRegistroJ2.getAcrescimoItem();
                lRegistrosJ2[iDxJ2].VALOR_LIQUIDO = lRegistroJ2.getValorLiquido();
                lRegistrosJ2[iDxJ2].TOTALIZADOR_PARCIAL = toByte(lRegistroJ2.getTotalizadorParcial(), 8);
                lRegistrosJ2[iDxJ2].CASAS_DECIMAIS_QTDE = toByte(lRegistroJ2.getCasasDecimaisValQtde(), 2);
                lRegistrosJ2[iDxJ2].CASAS_DECIMAIS_VAL_UNIT = toByte(lRegistroJ2.getCasasDecimaisValUnit(), 2);
                lRegistrosJ2[iDxJ2].NUMERO_NOTA = toByte(lRegistroJ2.getNumeroNota(), 11);
                lRegistrosJ2[iDxJ2].SERIE_NOTA = toByte(lRegistroJ2.getSerieNota(), 4);
                lRegistrosJ2[iDxJ2].CHAVE_NF = toByte(lRegistroJ2.getChaveNf(), 45);
                lRegistrosJ2[iDxJ2].TIPO_DOC = toByte(lRegistroJ2.getTipoDoc(), 2);
                lRegistrosJ2[iDxJ2].RegistroValido = converteBooleanToByte(lRegistroJ2.isRegistroValido());
                iDxJ2++;                
            }
            
            iDxJ1++;
        }
        int ret = ACBrPAFInterop.INSTANCE.PAF_Preenche_J(getHandle(), lRegistrosJ1, paf_J.getRegistrosJ1().size(), lRegistrosJ2);
        checkResult(ret);
    }
    ...
    private void preenche_P() throws ACBrException {
        ...
        for (ACBrPAFRegistroP2 lRegistro : paf_P.getRegistrosP2()) {
            ...
            lRegistrosP2[i].CEST = toByte(lRegistro.getCest(), 8);
            lRegistrosP2[i].NCM = toByte(lRegistro.getNcm(), 9);
