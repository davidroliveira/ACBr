...
uses
  ...
  ACBrPosPrinter, ACBrBase, ACBrDevice, FMX.ComboEdit, System.Bluetooth,
...
type
  ...
  TFrPosPrinterTeste = class(TForm)
    ...
    procedure cbxTestesChange(Sender: TObject);
    procedure btLerInformacoesClick(Sender: TObject);
    procedure btLerStatusClick(Sender: TObject);
  private
    ...
    function CalcularPathTrabalho: String;
...
begin
  {$IfDef ANDROID}
   ...
   SharedDocumentsPath := TPath.GetSharedDocumentsPath;
   LibraryPath := TPath.GetLibraryPath;
   CachePath := TPath.GetCachePath;
   PublicPath := TPath.GetPublicPath;
   PicturesPath := TPath.GetPicturesPath;
   DownloadsPath := TPath.GetDownloadsPath;
   CameraPath := TPath.GetCameraPath;

   Result := TPath.Combine(TPath.GetHomePath, 'ACBrPosPrinter.ini' );
  {$ElseIf Defined(FMX) and Defined(POSIX) and Defined(DEBUG)}
   // Salva no diretÛrio anterior, pois o PAServer sempre apaga a Pasta antes de executar
...
function TFrPosPrinterTeste.CalcularPathTrabalho: String;
begin
  {$IfDef ANDROID}
...
procedure TFrPosPrinterTeste.cbxTestesChange(Sender: TObject);
var
  BmpMono: String;
begin
  ...
  case cbxTestes.ItemIndex of
    0:    // Tags de Format. de Caracter
    begin
      mImp.Lines.Add('</zera>');
    ...
    1:    // Tags de Alinhamento
    begin
      BmpMono := CalcularPathTrabalho+'acbrmono.bmp';
...
begin
  if pos(PathDelim,edLog.Text) = 0 then
    AFileLog := CalcularPathTrabalho + edLog.Text
  else
    AFileLog := edLog.Text;
...
begin
  {$IfNDef MSWINDOWS}
  MessageDlg('DisponÌvel apenas em MS-Windows', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
  {$Else}
...
procedure TFrPosPrinterTeste.btLerInformacoesClick(Sender: TObject);
begin
  mImp.Lines.Add( ACBrPosPrinter1.LerInfoImpressora );
end;

procedure TFrPosPrinterTeste.btLerStatusClick(Sender: TObject);
begin
  LerStatus;
end;
...
begin
  ...
  begin
    ...
    try
      Image1.MultiResBitmap.Items[0].Bitmap.SaveToStream(MS);
      MS.Position := 0;
...
procedure TFrPosPrinterTeste.LerStatus;
var
...
begin
  ...
  begin
    ...
    try
      ACBrPosPrinter1.Modelo := TACBrPosPrinterModelo( cbxModelo.ItemIndex );
      ACBrPosPrinter1.Porta  := cbxPorta.Text;
      ACBrPosPrinter1.ArqLOG := CalcularPathTrabalho + edLog.Text;
      ACBrPosPrinter1.LinhasBuffer := Trunc(seLinhasBuffer.Value);
      ACBrPosPrinter1.LinhasEntreCupons := Trunc(seLinhasPular.Value);
      ACBrPosPrinter1.EspacoEntreLinhas := Trunc(seEspLinhas.Value);
      ACBrPosPrinter1.ColunasFonteNormal := Trunc(seColunas.Value);
