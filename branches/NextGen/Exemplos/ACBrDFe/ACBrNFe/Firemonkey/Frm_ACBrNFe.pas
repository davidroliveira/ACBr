...
var
  ...
  PFXFile: string;
begin
  //PFXFile := '/storage/emulated/0/Pictures/acbr.pfx';
...
//  if not FileExists(PFXFile) then
//    PFXFile := TPath.Combine(TPath.GetSharedDocumentsPath, 'cert.txt');
//  if not FileExists(PFXFile) then
//    PFXFile := TPath.Combine(TPath.GetHomePath, 'cert.txt');
//  if not FileExists(PFXFile) then
//    PFXFile := TPath.Combine(TPath.GetPublicPath, 'cert.txt');
//  ACBrNFe1.Configuracoes.Certificados.ArquivoPFX  := PFXFile;
//  ACBrNFe1.Configuracoes.Certificados.Senha       := '123';
