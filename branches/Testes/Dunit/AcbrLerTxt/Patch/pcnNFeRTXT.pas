...
begin
  ...
  eIdB := StringToLayid( FID );

  case eIdB of
    IdB    :  begin (* Grupo da TAG <ide> **************************************)
                ...
                begin
                  ...
                  (*B10 *)NFe.ide.dSaiEnt  := LerCampo(tcDat, 'dhSaiEnt');
    ...
    IdBA19,
    IdBA20 :  begin
                NFe.Ide.NFref.Add; (* Grupo da TAG <ide><NFref><refNFe> ****************************************)
                i := NFe.ide.NFref.Count - 1;
                case eIdB of
                  IdBA02  : (*B13*)NFe.ide.NFref[i].refNFe := LerCampo(tcEsp, 'refNFe');
                  IdBA03  : begin(* Grupo da TAG <ide><NFref><RefNF> **********************)
                  ...
                  IdBA10  : begin (* Grupo da TAG <ide><NFref><refNFP> **********************)
                  ...
                  IdBA13  : (*B20d*)NFe.Ide.NFref[i].refNFP.CNPJCPF := LerCampo(tcEsp, 'CNPJ');
                  IdBA14  : (*B20e*)NFe.Ide.NFref[i].refNFP.CNPJCPF := LerCampo(tcEsp, 'CPF');
                  IdBA19  : {*BA19*}NFe.Ide.NFref[i].refCTe         := LerCampo(tcEsp, 'refCTe');
                  IdBA20  : begin(* Grupo da TAG <ide><NFref><refECF> **********************)
    ...
    IdC02   :   (*C02 *)NFe.Emit.CNPJCPF := LerCampo(tcStr, 'CNPJ');
    IdC02A  :   (*C02A*)NFe.Emit.CNPJCPF := LerCampo(tcStr, 'CPF');
    IdC05   : begin (* Grupo da TAG <emit><EnderEmit> ************************)
    ...
    IdE02   : (*E02*)NFe.Dest.CNPJCPF := LerCampo(tcStr, 'CNPJ');
    IdE03   : (*E03*)NFe.Dest.CNPJCPF := LerCampo(tcStr, 'CPF');
    IdE03A  :(*E03a*)NFe.Dest.idEstrangeiro := LerCampo(tcStr, 'idEstrangeiro');
    IdE05   : begin  (* Grupo da TAG <dest><EnderDest> ************************)
    ...
    IdF     : begin (* Grupo da TAG <retirada> *********************************)
    ...
    IdF02   :  (*F02 *)NFe.Retirada.CNPJCPF := LerCampo(tcStr, 'CNPJ');
    IdF02A  :  (*F02a*)NFe.Retirada.CNPJCPF := LerCampo(tcStr, 'CPF');
    IdG     : begin (* Grupo da TAG <entrega> **********************************)
    ...
    IdG02   : (*G02*)NFe.Entrega.CNPJCPF    := LerCampo(tcStr, 'CNPJ');
    IdG02A  :(*G02a*)NFe.Entrega.CNPJCPF    := LerCampo(tcStr, 'CPF');
    IdGA02  :        NFe.autXML.Add.CNPJCPF := LerCampo(tcStr, 'CNPJ');
    IdGA03  :        NFe.autXML.Add.CNPJCPF := LerCampo(tcStr, 'CPF');
    IdH     : begin
                       NFe.Det.Add;
                  i := NFe.Det.Count - 1;
    ...
    IdLA,   IdLA07,
    IdLB,
    IdL109, IdL114, IdL117,
    IdM,

    IdN02,  IdN03,  IdN04,  IdN05,  IdN06,  IdN07,  IdN08,  IdN09,
    IdN10,  IdN10A, IdN10B, IdN10C, IdN10D, idN10E, IdN10F, IdN10G, IdN10H,

    IdO,    IdO07,  IdO08,  IdO10,  IdO11,
    IdP,
    IdQ02,  IdQ03,  IdQ04,  IdQ05,  IdQ07,  IdQ10,
    IdR,    IdR02,  IdR04,
    IdS02,  IdS03,  IdS04,  IdS05,  IdS07,  IdS09,
    IdT,    IdT02,  IdT04,
    idNA,
    IdU,
    IdUA    : begin (* Grupo da TAG <det> **************************************)
                i := NFe.Det.Count - 1;
                case eIdB of
                  IdI     : begin (* Grupo da TAG <det><prod> ********************************)
                             ...
                             (*I10a*)NFe.Det[i].Prod.vUnCom   := LerCampo(tcDe10, 'vUnCom');
                             ...
                             (*I14a*)NFe.Det[i].Prod.vUnTrib  := LerCampo(tcDe10, 'vUnTrib');
                  ...
                  IdI05a  : (*I05a*)NFe.Det[i].Prod.NVE.Add.NVE := LerCampo(tcStr, 'NVE');
                  IdI05c  : (*I05c*)NFe.Det[i].Prod.CEST        := LerCampo(tcStr, 'CEST');

                  IdI18   : begin (* Grupo da TAG <det><prod><DI> **************************)
                  ...
                  IdI25   : begin (* Grupo da TAG <det><prod><DI><adi> *********************)
                                j := NFe.Det[i].Prod.DI.Count - 1;
                  ...
                  IdI50   : begin (* Grupo da TAG <det><prod><detExport> **************************)
                              (*I50*)NFe.Det[i].Prod.detExport.Add.nDraw      := LerCampo(tcStr, 'nDraw');
                            end;

                  IdI52   : begin (* Grupo da TAG <det><prod><detExport> **************************)
                  ...
                  IdJ     : begin (* Grupo da TAG <det><prod><veicProd> **********************)
                  ...
                  IdK     : begin (* Grupo da TAG <det><prod><med> ***************************)
                  ...
                  IdL     : begin (* Grupo da TAG <det><prod><arma> **************************)
                  ...
                  IdLA    : begin (* Grupo da TAG <det><prod><comb> ************************)
                  ...
                  IdLA07  : begin (* Grupo da TAG <det><prod><comb><CIDE> *****************)
                  ...
                  IdLB    : NFe.Det[i].Prod.nRECOPI := LerCampo(tcStr, 'nRECOPI');

                  IdL109  : begin (* Grupo da TAG <det><prod><comb><ICMS> *****************)
                  ...
                  IdL114  : begin (* Grupo da TAG <det><prod><comb><ICMSInter> ************)
                  ...
                  IdL117  : begin (* Grupo da TAG <det><prod><comb><ICMSCons> *************)
                  ...
                  IdM     : begin (* Grupo da TAG <det><imposto> ********************)
                              (*M02*)NFe.Det[i].Imposto.vTotTrib:= LerCampo(tcDe2, 'vTotTrib');
                            end;


                  IdN10D  : begin (* Grupo da TAG <det><imposto> ********************)
                  ...
                  IdN02,  IdN03 ,  IdN04 ,  IdN05,
                  IdN06,  IdN07 ,  IdN08 ,  IdN09,
                  IdN10,IdN10A,  IdN10B,  IdN10C,
                  idN10E,  IdN10F,  IdN10G,
                  IdN10H : begin (* Grupo da TAG <det><imposto><ICMS> ********************)
                              (*N11*)NFe.Det[i].Imposto.ICMS.orig         := StrToOrig(ok, LerCampo(tcStr, 'orig'));
                             ...
                             (*N12a*)NFe.Det[i].Imposto.ICMS.CSOSN        := StrToCSOSNICMS(ok, LerCampo(tcStr, 'CSOSN'));
                              ...
                              (*N32*)NFe.Det[i].Imposto.ICMS.vICMSDeson   := LerCampo(tcDe2, 'vICMSDeson');
                                     NFe.Det[i].Imposto.ICMS.vICMSSTDest  := LerCampo(tcDe2, 'vICMSSTDest');
                                     NFe.Det[i].Imposto.ICMS.vICMSOp      := LerCampo(tcDe2, 'vICMSOp');
                                     NFe.Det[i].Imposto.ICMS.pDif         := LerCampo(tcDe2, 'pDif');
                                     NFe.Det[i].Imposto.ICMS.vICMSDif     := LerCampo(tcDe2, 'vICMSDif');
                           end;

                  IdO    : begin (* Grupo da TAG <det><imposto><IPI> **********************)
                  ...
                  IdQ02  : begin (* Grupo da TAG <det><imposto><pis><pisaliq> *************)
                  ...
                  IdQ03  : begin (* Grupo da TAG <det><imposto><pis><pisqtde> *************)
                              (*Q06*)NFe.Det[i].Imposto.PIS.CST       := StrToCSTPIS(ok, LerCampo(tcStr, 'CST'));
                  ...
                  IdQ04  : begin (* Grupo da TAG <det><imposto><pis><pisNT> *************)
                              (*Q06*)NFe.Det[i].Imposto.PIS.CST := StrToCSTPIS(ok, LerCampo(tcStr, 'CST'));
                           end;

                  IdQ05  : begin (* Grupo da TAG <det><imposto><pis><pisPOutr> *************)
                              (*Q06*)NFe.Det[i].Imposto.PIS.CST := StrToCSTPIS(ok, LerCampo(tcStr, 'CST'));
                              (*Q09*)NFe.Det[i].Imposto.PIS.vPIS := LerCampo(tcDe2, 'vPIS');
                           end;

                  IdQ07  : begin (* Grupo da TAG <det><imposto><pis><pisqtde> *************)
                              (*Q07*)NFe.Det[i].Imposto.PIS.vBC := LerCampo(tcDe2, 'vBC');
                              (*Q08*)NFe.Det[i].Imposto.PIS.pPIS := LerCampo(tcDe2, 'pPIS');
                              if (nfe.infNFe.Versao < 3.10) then
                                (*Q09*)NFe.Det[i].Imposto.PIS.vPIS := LerCampo(tcDe2, 'vPIS');
                  ...
                  IdS03  : begin (* Grupo da TAG <det><imposto><COFINS> *******************)
                              (*S06*)NFe.Det[i].Imposto.COFINS.CST      := StrToCSTCOFINS(ok, LerCampo(tcStr, 'CST'));
                              (*S07*)NFe.Det[i].Imposto.COFINS.vBC      := LerCampo(tcDe2, 'vBC');
                  ...
                  IdS04  : begin (* Grupo da TAG <det><imposto><COFINS> *******************)
                              (*S06*)NFe.Det[i].Imposto.COFINS.CST := StrToCSTCOFINS(ok, LerCampo(tcStr, 'CST'));
                           end;

                  IdS05  : begin (* Grupo da TAG <det><imposto><COFINS> *******************)
                              (*S06*)NFe.Det[i].Imposto.COFINS.CST     := StrToCSTCOFINS(ok, LerCampo(tcStr, 'CST'));
                              (*S11*)NFe.Det[i].Imposto.COFINS.vCOFINS := LerCampo(tcDe2, 'vCOFINS');
                           end;

                  IdS07  : begin (* Grupo da TAG <det><imposto><COFINS> *******************)
                              (*S07*)NFe.Det[i].Imposto.COFINS.vBC := LerCampo(tcDe2, 'vBC');
                              (*S08*)NFe.Det[i].Imposto.COFINS.pCOFINS := LerCampo(tcDe2, 'pCOFINS');
                           end;

                  IdS09  : begin (* Grupo da TAG <det><imposto><COFINS> *******************)
                              (*S09*)NFe.Det[i].Imposto.COFINS.qBCProd := LerCampo(tcDe4, 'qBCProd');
                              (*S10*)NFe.Det[i].Imposto.COFINS.vAliqProd := LerCampo(tcDe4, 'vAliqProd');
                           end;

                  IdT    : begin (* Grupo da TAG <det><imposto><COFINSST> *******************)
                              (*T06*)NFe.Det[i].Imposto.COFINSST.vCOFINS := LerCampo(tcDe2, 'vCOFINS');
                           end;

                  IdT02  : begin (* Grupo da TAG <det><imposto><COFINSST> *****************)
                              (*T02*)NFe.Det[i].Imposto.COFINSST.vBC := LerCampo(tcDe2, 'vBC');
                              (*T03*)NFe.Det[i].Imposto.COFINSST.pCOFINS := LerCampo(tcDe2, 'pCOFINS');
                           end;

                  IdT04  : begin (* Grupo da TAG <det><imposto><COFINSST> *****************)
                  ...
                  IdU    : begin (* Grupo da TAG <det><imposto><ISSQN> **********************)
                              ...
                              (*U06*)NFe.Det[i].Imposto.ISSQN.cListServ     := LerCampo(tcStr, 'cListServ');
                                     ...
                                     NFe.Det[i].Imposto.ISSQN.cMun          := LerCampo(tcInt, 'cMun');
                                     NFe.Det[i].Imposto.ISSQN.cPais         := LerCampo(tcInt, 'cPais');
                                     NFe.Det[i].Imposto.ISSQN.nProcesso     := LerCampo(tcStr, 'nProcesso');
                                     NFe.Det[i].Imposto.ISSQN.indIncentivo  := StrToindIncentivo(Ok,LerCampo(tcStr, 'indIncentivo'));
                              (*U07*)NFe.Det[i].Imposto.ISSQN.cSitTrib      := StrToISSQNcSitTrib(Ok, LerCampo(tcStr, 'cSitTrib'));
                           end;

                  IdUA   : begin  (* Grupo da TAG <det><impostoDevol> **********************)
    ...
    IdW02   : begin (* Grupo da TAG <total><ICMSTot> *************************)
    ...
    IdW04c  : (*W04c*)NFe.Total.ICMSTot.vFCPUFDest   := LerCampo(tcDe2, 'vFCPUFDest');
    IdW04e  : (*W04e*)NFe.Total.ICMSTot.vICMSUFDest  := LerCampo(tcDe2, 'vICMSUFDest');
    IdW04g  : (*W04g*)NFe.Total.ICMSTot.vICMSUFRemet := LerCampo(tcDe2, 'vICMSUFRemet');
    IdW17   : begin (* Grupo da TAG <total><ISSQNtot> ************************)
    ...
    IdW23   : begin (* Grupo da TAG <total><retTrib> *************************)
    ...
    IdX     : (*X02*)NFe.Transp.modFrete := StrTomodFrete(ok, LerCampo(tcStr, 'modFrete'));
    IdX03   : begin (* Grupo da TAG <transp><TRansportadora> *****************)
    ...
    IdX04   : (*X04*)NFe.Transp.Transporta.CNPJCPF  := LerCampo(tcStr, 'CNPJ');
    IdX05   : (*X05*)NFe.Transp.Transporta.CNPJCPF  := LerCampo(tcStr, 'CPF');

    IdX11   : begin (* Grupo da TAG <transp><retTransp> **********************)
    ...
    IdX18   : begin (* Grupo da TAG <transp><veicTransp> *********************)
    ...
    IdX22   : begin (* Grupo da TAG <transp><reboque> ************************)
...
//    IdX25a  : {*X25a}
//    IdX25b  : {*X25b}
    IdX26   : begin (* Grupo da TAG <transp><vol> ****************************)
                       NFe.Transp.Vol.add;
    ...
    IdX33   : begin  (* Grupo da TAG <transp><vol><lacres> ********************)
                  i := NFe.Transp.Vol.Count - 1;
    ...
    IdY07   : begin (* Grupo da TAG <cobr><dup> ******************************)
                     NFe.Cobr.Dup.Add;
    ...
    IdYA04a : begin
                i := NFe.pag.Count - 1;
            (*YA04a*)NFe.pag[i].tpIntegra := StrTotpIntegra(ok, LerCampo(tcStr, 'tpIntegra'));
              end;
    IdZ     : begin (* Grupo da TAG <infAdic><procRef> ***********************)
                (**)NFe.InfAdic.infAdFisco := LerCampo(tcStr, 'InfAdic');
                (**)NFe.InfAdic.infCpl     := LerCampo(tcStr, 'infCpl');
              end;

    IdZ10   : begin (* Grupo da TAG <infAdic><procRef> ***********************)
    ...
    IdZA    : begin (* Grupo da TAG <exporta> *********************************)
                if (nfe.infNFe.Versao < 3.10 ) then
                begin
                ...
                begin
                  (*ZA02*)NFe.exporta.UFSaidaPais   := LerCampo(tcStr, 'UFSaidaPais');
                  (*ZA03*)NFe.exporta.xLocExporta   := LerCampo(tcStr, 'xLocExporta');
                  (*ZA03*)NFe.exporta.xLocDespacho  := LerCampo(tcStr, 'xLocDespacho');
                end;
              end;


    IdZB    : begin  (* Grupo da TAG <compra> **********************************)
    ...
    IdZC   : begin  //Precisa rever todo esse grupo e tbm ZC04  ZC10
