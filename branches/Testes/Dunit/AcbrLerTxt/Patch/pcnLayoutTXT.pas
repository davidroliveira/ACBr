...
begin
  ...
  begin
    ...
    LoadLayout('<B01>     B|cUF¨|cNF¨|NatOp¨|indPag¨|mod¨|serie¨|nNF¨|dhEmi¨|dhSaiEnt¨|hSaiEnt¨|tpNF¨|cMunFG¨|TpImp¨|TpEmis¨|CDV¨|TpAmb¨|FinNFe¨|ProcEmi¨|VerProc¨|dhCont¨'); //ok
    LoadLayout('<B12a>  B13|refNFe¨'); //ok
    LoadLayout('<B14>   B14|cUF¨|AAMM¨|CNPJ¨|Mod¨|serie¨|nNF¨'); //ok
  ...
  begin
    ...
    LoadLayout('<BA10> BA10|cUF¨|AAMM¨|IE¨|Mod¨|serie¨|nNF¨');
    LoadLayout('<BA13> BA13|CNPJ¨');
    LoadLayout('<BA14> BA14|CPF¨');
    LoadLayout('<BA19> BA19|refCTe¨');
    ...
    LoadLayout('<UA>     UA|PDevol¨|VIPIDevol¨');
    LoadLayout('<W01>     W|'); //ok
    ...
    LoadLayout('<YA>     YA|TPag¨|VPag¨|CNPJ¨|TBand¨|CAut¨');
    LoadLayout('<YA>  YA04a|tpIntegra¨');
