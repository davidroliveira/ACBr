...
TACBrECFNaoFiscal = class( TACBrECFClass )
 private
    ...
    function AchaICMSAliquotaNaoFiscal(Aliquota: Double; Tipo : Char = ' '): Integer ;
...
begin
  ...
  case AliquotaICMS[1] of
   ...
   begin
     ...
     ValAliq := StrToFloatDef( AliquotaICMS, 0 ) ;
     PosAliq := AchaICMSAliquotaNaoFiscal( ValAliq, Tipo ) ;
     if (PosAliq < 0) and (ValAliq > 0) then
     begin
        AchaICMSAliquota( ValAliq, Tipo ) ;
        PosAliq := AchaICMSAliquotaNaoFiscal( ValAliq, Tipo ) ;
     end ;
   end ;
  end;
...
//N.Serie.: XXXXXXXXXXXXXXX  Maq. 999  Ver: 0.8.3b
//Operador: XXXXXXXXXXXXXXX     99/99/99 99:99:99V
  with fsBuffer do
  begin
     Add( StringOfChar('-',Colunas) ) ;
     Add( padS('N.Serie.: '+padL(fsNumSerie,15)+'|Maq '+padL(fsNumECF,3)+'|'+
               ' Ver '+ACBR_VERSAO,Colunas,'|') );
     Add( padS('Operador: '+padL(Operador,15)  +'|'+
...
begin
  ...
  begin
     For A := 0 to Count -1 do
        if (TACBrECFNaoFiscalAliquota(Items[A]).Aliquota = Aliquota) and
           ((Tipo = ' ') or
            (    TACBrECFNaoFiscalAliquota(Items[A]).Iss and (Tipo = 'S')) or
            (not TACBrECFNaoFiscalAliquota(Items[A]).Iss and (Tipo = 'T'))) then
         begin
           result := A ;
           Break ;
         end ;
