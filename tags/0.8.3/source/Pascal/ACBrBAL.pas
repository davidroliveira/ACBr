...
interface
uses ACBrBase, ACBrBALClass,  {Units da ACBr}
     SysUtils,
...
TACBrBALModelo = (balNenhum, balFilizolaBP, balToledo ) ;
TACBrBALLePeso = procedure(Peso : Double; Resposta : String) of object ;
...
implementation
Uses ACBrUtil, ACBrBALFilizolaBP, ACBrBALToledo, 
     {$IFDEF Delphi6_UP} StrUtils {$ELSE} ACBrD5{$ENDIF},
     Math;
...
begin
  ...
  case Value of
     balFilizolaBP    : fsBAL := TACBrBALFilizolaBP.create( Self ) ;
...
function TACBrBAL.GetUltimaResposta: String;
begin
...
begin
  ...
  fsTimer.Enabled  := ((Value <> 0) and fsMonitorarBalanca and Ativo);
end;
