...
{Constante para definir as Versoes}
   V0203 = '02.03';
   V0205 = '02.05';
...
//   fpDevice.Serial.DTR := false ;  { Desliga o DTR para ler a Resposta }
     ...
     if ErroMsg <> '' then
      begin
        ...
        Sleep(100) ;  { Pequena pausa entre comandos }
  finally
...
Var AliquotaStr, QtdStr, ValorStr, CodDescr, LinhaItem : String ;
    ValAliquota : Double ;
...
begin
  ...
  CodDescr := IntToStrZero( fsItensCupom.Count+1,3) + ' ' + 
  ...
  if fsNumVersao = V0203 then
     LinhaItem := CodDescr + AliquotaStr + QtdStr + ValorStr +
                  IntToStrZero( Round( DescontoPorc*100) ,4) 
  else
   begin
     ...
     LinhaItem := CodDescr + AliquotaStr + QtdStr + ValorStr +
                  IntToStrZero( Round( DescontoPorc*100),4) + UMD.Indice ;
   end;

   if fsItensCupom.Count = 0 then  { Primeiro Item, abre o cupom e demora mais }
    begin
      AguardaImpressao := True ;
      EnviaComando( 'A' + LinhaItem, 10) ;
    end
   else
      EnviaComando( 'A' + LinhaItem ) ;
...
begin
   ...
   if fsNumVersao = V0203 then
      ...
      EnviaComando( 't' + '01' + IfThen(PermiteVinculado,'T','N') + Descricao + '01');
...
begin
   ...
   raise Exception.Create('Impressora Dataregis não possui comandos para cancelar '+
                       'a impressão de cheques. ' + sLineBreak + 'Por favor desligue '+
                       'e ligue a impressora ou insira um documento.');
end;
...
begin
   ...
   {Impressora 02.05 pede o indice do relatorio gerencial, 00 é o default}
   if fsNumVersao = V0203 then
      EnviaComando('j' + padL( Linha,Colunas))
   else
      EnviaComando('j' + '00' + padL( Linha,Colunas));
end;
...
begin
   {Compativel com 02.03 e 02.05}
   if fsNumVersao = V0203 then
      EnviaComando( 'j' + padl(Linha,40), 15)
   else
      EnviaComando( 'J' + '00' + padL(Linha,40), 15);
end;
...
begin
   ...
   { Exceçao silenciosa, pois a Impressora pode nao estar em Estado}
   { de Relatorio. Na xxx não há flag para sinalizar Relatorio Aberto}
   try
      AguardaImpressao := True ;
      EnviaComando( 'k', 15 ) ;
   except
...
begin
   ...
   if fsNumVersao = V0203 then
    ...
    begin
      ...
      while StrRet <> '' do
      begin
         Descr := trim( UpperCase( copy(StrRet, 1, 20) ) ) ;
...
begin
  ...
  if (not FileExists( ArqTemp )) then
   begin
     ...
     begin
        Add('001BANCO DO BRASIL S/A     01750400060009001136') ;
        ...
        Add('038BANCO DO ESTADO DO PARAN01750505070509051123') ;
        ...
        Add('341BANCO ITAU S/A          01760400070010001334') ;
        ...
        Add('104CAIXA ECONOMICA FEDERAL 01750400060009001137') ;
        ...
        Add('399BANCO BAMERINDUS DO BRAS01750510071010101335') ;
     ...
     fsModelosCheque.LoadFromFile(fsArqPrgBcoTXT);
end;
...
begin
  ...
  begin
     ...
     begin
        ...
        UMD.Descricao := trim( UpperCase( copy(StrRet, 3, 2) ) );
...
begin
  ...
  try
     ...
     Ini.WriteString('ECF','COO',fsNumCupom) ;

     Ini.WriteFloat('Variaves','TotalPago',fsTotalPago);
     Ini.WriteFloat('Variaves','DescontoAcrescimo',fsDescontoAcrescimo);
...
begin
  ...
  if not FileExists(fsArqINI) then exit ;
  ...
  try
     if (Trim(Ini.ReadString('ECF','NumECF',''))   <> fsNumECF) or
        (Trim(Ini.ReadString('ECF','NumSerie','')) <> fsNumSerie) or
        (Trim(Ini.ReadString('ECF','COO',''))      <> NumCupom) then
        exit ;

     fsTotalPago         := Ini.ReadFloat('Variaves','TotalPago',0);
     fsDescontoAcrescimo := Ini.ReadFloat('Variaves','DescontoAcrescimo',0);
