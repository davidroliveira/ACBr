...
TACBrLCB = class( TACBrComponent )
  ...
  public
    ...
    procedure EnviarString( AString : String ) ;
...
uses ACBrUtil,
    {$IFDEF Delphi6_UP} DateUtils, StrUtils, {$ELSE} ACBrD5, {$ENDIF}
     Math, synaser;
...
procedure TACBrLCB.EnviarString(AString: String);
begin
  if not fsAtivo then
...
function TACBrLCB.LerString: String;
begin
  if not fsAtivo then
     raise Exception.Create('Componente ACBrLCB não está ATIVO');
