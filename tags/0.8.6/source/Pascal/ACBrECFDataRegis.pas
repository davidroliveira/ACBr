...
(*  if ArredondaPorQtd {and (not Arredonda)} then
  ...
  if TipoDescontoAcrescimo = '%' then
     ...
     DescStr := IntToStrZero( Round( DescontoPorc/(ValorUnitario*Qtd) * 100 * 100), 4) ;
...
begin
  ...
  case AliquotaICMS[1] of
    'I','N','F' :
       begin
          Result := AchaIndiceTipo( AliquotaICMS[1] ) ;
          if Result <> nil then
             AliquotaStr := Result.Indice
          else
...
begin
  Result := 0 ;
  if IsV03 then
     Result := strtointdef(copy(EnviaComando('gSN'),118,16),0)/100;
end;
