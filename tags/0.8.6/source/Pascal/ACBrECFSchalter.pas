...
Function TACBrECFSchalter.EnviaComando_ECF( cmd : String ) : String ;
Var ErroMsg : String ;
    Erro : Integer ;
begin
...
function TACBrECFSchalter.VerificaFimLeitura(Retorno:String) : Boolean;
begin
...
function TACBrECFSchalter.VerificaFimImpressao: Boolean;
...
begin
  ...
  begin
     ...
     fsNumVersao := RemoveString(',',fsNumVersao) ;
     fsNumVersao := RemoveString('.',fsNumVersao) ;
  end ;
...
(*  { SCHALTER TRUNCA quando PrecoUnit tem 3 ou mais decimais, com 2 decimais ela
  ...
  begin
     ...
     if TipoDescontoAcrescimo='%' then
        DescontoStr := IntToStrZero( Round(ValorUnitario*Qtd*DescontoPorc), 9)
     else
        DescontoStr := IntToStrZero( Round(DescontoPorc*100), 9) ;
   ...
   begin
     ...
     begin
        ...
        begin
           ...
           if IndiceFPG = -1 then
              raise Exception.Create('A impressora '+ModeloStr+' necessita como '+sLineBreak+
...
procedure TACBrECFSchalter.CorrigeEstadoErro;
begin
  try
     ZeraVinculados ;
  except
...
function TACBrECFSchalter.GetDadosUsuario: String;
  Var RetCmd : String ;
      I : Integer ;
begin
...
function TACBrECFSchalter.GetDadosLeituraX: String;
  Var RetCmd : String ;
begin
