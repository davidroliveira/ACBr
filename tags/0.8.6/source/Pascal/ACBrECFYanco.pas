...
function TACBrECFYanco.VerificaFimLeitura(Retorno:String) : Boolean;
begin
...
begin
  EnviaComando( '221             ',40) ;  { Espaço em branco = Supervisor C 13 }
end;
...
begin
  fsEmPagamento := true ;
  RetCmd	    := EnviaComando('34072');
  fsTotalAPagar := RoundTo( StrToIntDef(RetCmd,0) / 100,-2) ;
end;
...
begin
  ...
  begin
     if TipoDescontoAcrescimo='%' then
        DescontoStr := StringOfChar('0',12) + IntToStrZero( Round(DescontoPorc*100), 4) + '1'
     else
        DescontoStr := IntToStrZero( Round(DescontoPorc*1000), 12) + '0000' + '0' ;
