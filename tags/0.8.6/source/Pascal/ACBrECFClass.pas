...
//                write fpOnMsgErro ;
    ...
    Procedure VendeItem( Codigo, Descricao : String; AliquotaECF : String;
       ...
       Unidade : String = ''; TipoDescontoAcrescimo : string = '%') ; virtual ;
    ...
    Procedure RegistraItemNaoFiscal( CodCNF : String; Valor : Double;
       Obs : String = '') ; virtual ;
    Procedure SubtotalizaNaoFiscal( DescontoAcrescimo : Double = 0) ; virtual ;
...
begin
  ...
  if not (NewVar in ['T','S']) then
     raise Exception.create('Valores válidos para TACBrECFAliquota.Tipo:'+
                            '"T" - ICMS ou "S" - ISS') ;
  fsTipo := Value;
end;
...
begin
  ...
  if (Nome = '') and (Endereco <> '') then
     raise Exception.Create('Para informar o Endereço é necessário informar o Nome' ) ;

  fsDocumento := Trim( CPF_CNPJ );
  fsNome      := Trim( Nome );
  fsEndereco  := Trim( Endereco );
  fsEnviado   := False ;
end;
...
begin
  ...
  try
     ...
     try
        GravaLog(FormatDateTime('hh:nn:ss',now)+' '+fpComandoLOG);
        ...
        begin
           {$IFNDEF CONSOLE}
             ...
             begin
                ...
                if DoOnMsgRetentar('A impressora '+ModeloStr+' não está repondendo.',
                    'LerResposta') then
                   TempoLimite := IncSecond( now, TimeOut)  ;
             end ;
           {$ENDIF}
...
begin
  ...
  try
     ...
     if not DoOnMsgRetentar(
       'Comandos não estão sendo enviados para Impressora '+ModeloStr ,
...
begin
  ...
  begin
     {$IFNDEF CONSOLE}
       if Retentar and
          DoOnMsgRetentar('A impressora '+ModeloStr+' não está pronta.',
                          'VerEmLinha') then
          Continue ;
     {$ENDIF}
...
begin
  ...
  case Est of
     ...
     Msg := 'Estado da impressora '+ModeloStr+' é '+sLineBreak+
...
procedure TACBrECFClass.VendeItem(Codigo, Descricao, AliquotaECF: String;
...
begin
  raise EACBrECFCMDInvalido.create('Procedure: '+NomeProcedure + sLineBreak +
        ' não implementada para a Impressora: '+ModeloStr +sLineBreak + sLineBreak +
  ...
  case AliquotaICMS[1] of
    ...
    'F' : AliquotaICMS := 'FF' ;
     ...
     begin
        ...
        if Result = nil then
           raise EACBrECFCMDInvalido.Create('Aliquota não encontrada: '+AliquotaICMS)
     end ;
  end ;
