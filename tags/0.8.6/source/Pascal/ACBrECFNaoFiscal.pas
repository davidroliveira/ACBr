...
Function StuffMascaraItem( Linha, MascaraItem : String; Letra : Char;
       TextoInserir : String; Fim:Boolean = False) : String ;
...
Function TACBrECFNaoFiscal.EnviaComando_ECF( cmd : String ) : String ;
begin
...
begin
  ...
  fsBuffer.Add( fsCmdImpExpandidoUmaLinha + padC('LEITURA X', NumeroColunas() ) );
...
begin
  ...
  fsBuffer.Add( fsCmdImpExpandidoUmaLinha + padC('REDUCAO Z', NumeroColunas() ) );
...
begin
  ...
  fsBuffer.Add( fsCmdImpExpandidoUmaLinha +
		padC('*** CUPOM CANCELADO ***', NumeroColunas() ) ) ;
...
begin
  ...
  if PosAliq > 2 then
     AliquotaECF := padC(IfThen(Aliq.Iss,'S','T')+FormatFloat('#0.00',Aliq.Aliquota)+'%',7)
  else
  ...
  begin
     if TipoDescontoAcrescimo = '%' then
        ...
        ValDesc := RoundTo(DescontoPorc, -3) ;
...
begin
  ...
  fsBuffer.Add( fsCmdImpExpandidoUmaLinha +
		padC('RELATORIO GERENCIAL', NumeroColunas() )  );
  fsBuffer.Add( '' ) ;
