...
interface
uses ACBrBase, ACBrECFClass, ACBrRFD, {Units da ACBr}
...
TACBrECFModelo = (ecfNenhum, ecfNaoFiscal, ecfBematech, ecfSweda, ecfDaruma,
                  ...
                  ecfICash, ecfQuattro, ecfFiscNET );
...
//    function GetOnMsgErro: TACBrECFExibeErroEvent;
    ...
    function GetComandoLOGClass: String;
    procedure SetComandoLOGClass(const Value: String);
  ...
  public
    ...
    property ComandoLOG     : String  read GetComandoLOGClass
    ...
    Procedure NaoFiscalCompleto( CodCNF : String; Valor : Double;
       CodFormaPagto  : String; Obs : String ) ;
    ...
    Procedure EfetuaPagamentoNaoFiscal( CodFormaPagto : String; Valor : Double;
       Observacao : String = ''; ImprimeVinculado : Boolean = false) ;
    Procedure FechaNaoFiscal( Observacao : String = '') ;
  ...
  published
     ...
     property IntervaloAposComando : Integer read GetIntervaloAposComando
                 write SetIntervaloAposComando default cIntervaloAposComando ;
...
Uses ACBrUtil, ACBrECFBematech, ACBrECFNaoFiscal, ACBrECFDaruma, ACBrECFSchalter,
     ...
     ACBrECFICash, ACBrECFQuattro, ACBrECFFiscNET, ACBrECFSCU
    {$IFNDEF COMPILER6_UP} ,ACBrD5, Windows {$ELSE}, StrUtils {$ENDIF}, Math ;
...
begin
  if (Value < 0) or (Value > 3) then
     raise Exception.Create('Valor de DecimaisPreco deve estar entre 0-3');
...
begin
  if (Value < 0) or (Value > 4) then
     raise Exception.Create('Valor de DecimaisQtd deve estar entre 0-4');
...
function TACBrECF.GetComandoLOGClass: String;
begin
  Result := fsECF.ComandoLOG ;
end;

procedure TACBrECF.SetComandoLOGClass(const Value: String);
begin
...
{$IFNDEF CONSOLE}
     Linha, Buffer, StrQtd, StrPreco : String ;
     Total, ValDesc : Double ;
{$ENDIF}
begin
  ...
  if ValorUnitario <= 0 then
     raise EACBrECFCMDInvalido.Create(
           'Valor Unitario deve ser superior a 0.');
  ...
  ComandoLOG := 'VendeItem( '+Codigo+' , '+Descricao+' , '+
                     ...
                     FloatToStr(DescontoPorc)+' , '+Unidade+' , '+
  ...
  {$IFNDEF CONSOLE}
   ...
   begin
     ...
     begin
        ...
        MemoAdicionaLinha( '<table width=100%><tr>'+
           '<td align=left>DESCONTO '+FormatFloat('#0.00', DescontoPorc)+'%</td>'+
       ...
       'F' : AliquotaICMS := 'F1' ;
        ...
        if Aliquota <> nil then
           AliquotaICMS := IntToStrZero(Aliquota.Sequencia,2) + Aliquota.Tipo +
                           IntToStrZero(Trunc(Aliquota.Aliquota*100),4) 
        else
           AliquotaICMS := '' ;
     end ;

     { RFD aceita apenas desconto em Valor, convertendo... }
     if (DescontoPorc > 0) and (TipoDescontoAcrescimo = '%') then
        DescontoPorc := RoundTo( RoundTo(ValorUnitario*Qtd,-2) *
                                 DescontoPorc / 100              , -2 ) ;

     fsRFD.VendeItem( Codigo, Descricao, Qtd, ValorUnitario, Unidade,
                      DescontoPorc, AliquotaICMS ) ;
  end ;
...
begin
  ...
  fsECF.SubtotalizaNaoFiscal( DescontoAcrescimo );
...
procedure TACBrECF.FechaNaoFiscal(Observacao: String);
begin
  if (Observacao = '') then
     Observacao := fsMensagemRodape ;
...
procedure TACBrECF.PulaLinhas(NumLinhas: Integer);
begin
...
begin
  ComandoLOG := 'AchaFPGDescricao( '+Descricao+' )' ;
...
function TACBrECF.AchaCNFDescricao( Descricao: String):
   TACBrECFComprovanteNaoFiscal;
begin
  ComandoLOG := 'AchaCNFDescricao( '+Descricao+' )' ;
...
begin
  if fsAtivo then
     raise Exception.Create('Não é possível mudar ACBrECF.RFD com o componente ativo');
