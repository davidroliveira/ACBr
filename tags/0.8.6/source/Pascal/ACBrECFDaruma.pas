...
begin
  Result := (fpDevice.Serial.WaitingDataEx <= 0) and
            (LimpaRetorno(Retorno) <> '') ;

  // Na Leitura da Memoria Fiscal, aguarda até chegar todos os dados            
  if Result and (fsEsperaDados > 0) then
  begin
     Sleep( fsEsperaDados );
     Result := (fpDevice.Serial.WaitingDataEx <= 0)
  end ;
...
begin
  if fpMFD then
   begin
     ...
     fsTotalAPagar := RoundTo( StrToIntDef( copy(RetCmd,10,13),0 ) / 100, -2) ;
   end
  else
   begin
...
begin
  Obs := Observacao ;
  if (not fpMFD) and (not Consumidor.Enviado) then  { Comando de Ident.Consumidor não existe na MFD}
  begin
     ...
     try
        ...
        if fsNumVersao = '2000' then
         begin
           ...
           EnviaComando( ESC + #208 + StrConsumidor, 5)
         end 
        else
         begin
           ...
           EnviaComando( ESC + #201 + StrConsumidor, 5)
         end ;
...
Var QtdStr, ValorStr, DescontoStr, SepDec : String;
    LenQtd : Integer ;
    Cmd : Char ;
begin
  if Qtd > 99999 then
  ...
  if fpMFD then
   begin
     ...
     ValorStr := IntToStrZero( Round( ValorUnitario * power(10,fpDecimaisPreco)),
                               8 ) ;
     if TipoDescontoAcrescimo = '%' then
        DescontoStr := '0' + IntToStrZero( Round(DescontoPorc * 100), 4) +
                       StringOfChar('0',7)
     else
        DescontoStr := '1' + IntToStrZero( Round(DescontoPorc * 100), 11) ;
   ...
   begin
     ...
     if TipoDescontoAcrescimo = '%' then
        DescontoStr := '0' + '00000'+ IntToStrZero( Round(DescontoPorc * 100), 4)
     else
        DescontoStr := '1' + IntToStrZero( Round(DescontoPorc * 100), 9) ;
   ...
   begin
     ...
     if TipoDescontoAcrescimo = '%' then
        ...
        DescontoStr := IntToStrZero( Round( DescontoPorc/(ValorUnitario*Qtd) * 100 * 100), 4) ;
     ...
     EnviaComando( ESC + Cmd + AliquotaECF + Codigo +
                   ifThen(Cmd = #223,'0','0000') + DescontoStr +
...
Var Espera : Integer ;
begin
  Espera := Trunc(TimeOut + ((ReducaoFinal - ReducaoInicial)/3) ) ;
  fsEsperaDados := 500 ;
  Linhas.Clear ;
...
Var Espera : Integer ;
begin
  Espera := Trunc(TimeOut + (DaysBetween(DataInicial,DataFinal)/3) ) ;
  fsEsperaDados := 500 ;
  Linhas.Clear ;
  Linhas.Text := EnviaComando(ESC + #209 + 's' +
...
begin
  ...
  Espera := Trunc(max(TimeOut,600) + (COOFinal - COOInicial) );
  fsEsperaDados := 1000;
  Linhas.Clear;
  AguardaImpressao := True;
  Linhas.Text := EnviaComando(FS + 'R' + #201 + '023' +
...
Var RetCmd : String ;
begin
  { Nota: não encontrado a leitura do ultimo item na MF 345 }
  Result := 0 ;
...
begin
   ...
   if fpMFD then
    begin
      RetCmd := EnviaComando( FS + 'R' + #201 + '009');
      RetCmd := Copy(RetCmd, 6, Length(RetCmd));
      for A := 0 to fpAliquotas.Count-1 do
      begin
        fpFormasPagamentos[A].Total := RoundTo( StrToFloatDef(Copy(RetCmd,(A*13)+1,13),0)
                                         / 100, -2 );
      end;
    end
   else if fsNumVersao = '2000' then
   ...
   else if (StrToInt(fsNumVersao) >= 345) {provisório} or fpMFD {provisório} then
    begin
      RetCmd := Ret244 ;
