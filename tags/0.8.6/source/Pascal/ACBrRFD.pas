...
TACBrRFDCupom = class
  ...
  public
    ...
    Procedure VendeItem( Codigo, Descricao: String;
       ...
       const ValorDesconto: Double; Aliquota: String) ;
...
TACBrRFD = class( TACBrComponent )     { Componente ACBrRFD }
  ...
  published
    ...
    Procedure ReducaoZ( DadosReducaoZ : String ) ;
...
begin
  ...
  ItemCupom.Desconto  := abs(ValorDesconto) ;
...
begin
  ...
  Linha := IntToStrZero( CCF, 6) +
           IntToStrZero( COO, 6) +
           DtoS( DataEmissao ) +
           IntToStrZero( Trunc(SubTotal *100), 14) +
           IntToStrZero( Trunc(Desconto *100), 13) + TipoDesconto  +
           IntToStrZero( Trunc(Acrescimo*100), 13) + TipoAcrescimo +
           IntToStrZero( Trunc(TotalLiquido*100), 14) +
           Cancelado +
  ...
  begin
     ...
     begin
        Linha := IntToStrZero( COO, 6) +
                 ...
                 padL(Codigo,14) +
                 padL(Descricao,100) +
                 Poem_Zeros( StringReplace( Trim( FloatToStr(Qtd) ),
                              DecimalSeparator, '', [rfReplaceAll] ) ,7 ) +
                 PadL( Unidade, 3) +
                 Poem_Zeros( StringReplace( Trim( FloatToStr(ValorUnitario) ),
                              DecimalSeparator, '', [rfReplaceAll] ) ,8 ) +
...
begin
  ...
  begin
     fsECF_NumSerie := NumSerie ; // Joga em variavel para evitar chamadas ao ECF
     ...
     if (Modelo = ecfSchalter) then    { Schalter demora quase 1 minuto para }
      begin                            { ler o CNPJ e IE... Soluçao provisoria }
        wCNPJ := '' ;                  { até termos um posicionamento da Elgin }
        wIE   := '' ;
        wCRO  := 0 ;
      end
     else
...
begin
  ...
  with TACBrECF( fsECF ) do
  begin
     fsCupom.TotalLiquido := Subtotal ;
     fsCupom.SubTotal     := fsCupom.TotalLiquido - DescontoAcrescimo ;
  end ;
...
begin
  if fsCupom.fsCOO = COO then
   ...
   begin
     { TODO: Abrir arquivo E14.TXT e verificar se ultima linha é o numero do COO
             Se for, altera-la para Cencelado.
         Motivo: Se usuário emitir um Vinculado, o método "Documento" descarrega
                 o cupom... porém o mesmo ainda pode ser cancelado se cancelar
                 o vinculado e depois cancelar o cupom
     } 
   end ;
end;
...
begin
  ...
  begin
     ...
     if Denominacao = 'RG' then
        ...
        CDC := IntToStrZero( StrToIntDef(NumCDC,0), 6)
...
procedure TACBrRFD.ReducaoZ( DadosReducaoZ : String ) ;
  Var Ini : TIniFile ;
      CRZ, COO : Integer ;
begin
  fsCupom.Descarrega ;
  ...
  with TACBrECF( fsECF ) do
  begin
     Ini := TIniFile.Create( ArqReducaoZ );
     try
...
begin
  ...
  if not ( FileExists(fsDirECFLog + PathDelim + cRFDArqCupom) or
           ...
           ( (fsArqReducaoZ <> '') and
             FileExists(fsDirECFLog + PathDelim + fsArqReducaoZ) )   )  then
     exit ;
  ...
  try
     ...
     begin
        ...
        Linha := 'E01' +
                 ...
                 wMarca + wModelo +
                 PadL( NumVersao, 10) +
                 StringOfChar(' ',14) +  // Data e Hora Gravação do Sw.Básico
     ...
     Linha := 'E02' +
              ...
              IntToStrZero( Round( fsCONT_GTCadastro*100), 18 ) +
              wNumUsuario ;
     WriteToTXT( ArqTmp, Linha, True);
     ...
     begin
        ...
        try
           ...
           begin
              P := pos('=',SEC[I]) ;
              if P > 0 then
                 SL.Add( PadL(copy(SEC[I],1,P-1),7)+
