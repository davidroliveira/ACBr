...
Function TACBrECFICash.VerificaFimLeitura(var Retorno:String;
...
Begin
  ...
  CS1 := chr( Trunc(iSoma / 256 ) ) ;
  CS2 := chr( iSoma mod 256 ) ;
...
begin
  ...
  EnviaComando( '54'+IntToStrZero(ReducaoInicial,4)+
                     IntToStrZero(ReducaoFinal  ,4)+'1', Espera ) ;
end;
...
begin
  ...
  EnviaComando( '55'+FormatDateTime('ddmmyyyy',DataInicial)+
                     FormatDateTime('ddmmyyyy',DataFinal)+'1', Espera ) ;
end;
...
procedure TACBrECFICash.LeituraMemoriaFiscalSerial(DataInicial,
 ...
 Var RetCmd : String ;
begin
  { ICash não permite informar Inicio e Fim na leitura pela Serial }
...
procedure TACBrECFICash.FechaNaoFiscal(Observacao: String);
Var Linhas    : TStringList ;
    I         : Integer ;
    Obs1, Obs2: String ;
begin
...
procedure TACBrECFICash.Sangria( const Valor: Double; Obs,
...
procedure TACBrECFICash.Suprimento(const Valor: Double; Obs, DescricaoCNF,
  DescricaoFPG: String);
begin
...
function TACBrECFICash.LimpaStr(AString: String): String;
Var A,Len : Integer ;
begin
  Result := '' ;
  Len    := Length( AString ) ;
