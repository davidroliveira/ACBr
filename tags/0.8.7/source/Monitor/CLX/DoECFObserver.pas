...
//***********Métodos de Cupom Fiscal*****************//
  ...
  begin
     ...
     begin
        ...
        Desconto := 0 ;
     Result := 'ECF.VendeItem("'+Parametro(Comando,2)+'", "'+Parametro(Comando,3)+'", "'+Parametro(Comando,4)+'", '+Parametro(Comando,6)+', '+FloatToStr(StrToFloatDef(Parametro(Comando,8),0))+' ,'+FloatToStr(Desconto)+', "'+Parametro(Comando,5)+'" ) ';
     Exit ;
  end;
  ...
  begin
     ...
     {$IFNDEF CONSOLE}
        FPG := FrmACBrMonitor.ACBrECF1.AchaFPGDescricao( Parametro(Comando,2) ) ;
     {$ELSE}
        FPG := dm.ACBrECF1.AchaFPGDescricao( Parametro(Comando,2) ) ;
     {$ENDIF}
