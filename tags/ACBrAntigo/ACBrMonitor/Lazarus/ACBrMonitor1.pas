...
|* 24/10/2008: Daniel Simoes de Almeida , Wagner Bueno Angelo
|*    Migraçao para Lazarus 0.9.26
******************************************************************************}
...
uses
  ...
  ComCtrls, ExtCtrls, Buttons, StdCtrls, Menus, Spin, LCLType, TypInfo,
  ConfiguraSerial, Sobre,
  {$IFDEF MSWINDOWS} sndkey32, {$ENDIF}
  ACBrECF, ACBrECFClass, ACBrDIS, ACBrGAV, ACBrGAVClass, ACBrBase, ACBrDevice,
  ACBrCHQ, ACBrLCB, ACBrRFD, ACBrBAL, ACBrETQ, ACBrECFNaoFiscal,
  CmdUnit, MaskEdit, LMessages, ACBrSocket, blcksock;                                    { Unit do ACBr }
...
type
  ...
  TFrmACBrMonitor = class(TForm)
    ...
    ACBrTCPServerTC: TACBrTCPServer ;
    ...
    Label61: TLabel;
    ACBrETQ1: TACBrETQ;
    tsTerminalConsulta: TTabSheet;
    gbTC: TGroupBox;
    Label62: TLabel;
    edTCPort: TEdit;
    Label63: TLabel;
    seTCConexoes: TSpinEdit;
    mTC: TMemo;
    cbxTCAtivo: TCheckBox;
    BitBtn1: TBitBtn;
    procedure ACBrTCPServer1Conecta( const TCPBlockSocket: TTCPBlockSocket ;
       var Enviar: String) ;
    procedure ACBrTCPServer1DesConecta( const TCPBlockSocket: TTCPBlockSocket ;
       Erro: Integer ; ErroDesc: String) ;
    procedure ACBrTCPServer1RecebeDados(
       const TCPBlockSocket: TTCPBlockSocket ; const Recebido: AnsiString ;
       var Enviar: String) ;
    procedure ACBrTCPServerTCConecta( const TCPBlockSocket: TTCPBlockSocket ;
       var Enviar: String) ;
    procedure ACBrTCPServerTCDesConecta(
       const TCPBlockSocket: TTCPBlockSocket ; Erro: Integer ; ErroDesc: String
       ) ;
    procedure ACBrTCPServerTCRecebeDados(
       const TCPBlockSocket: TTCPBlockSocket ; const Recebido: AnsiString ;
       var Enviar: String) ;
    ...
    procedure cbxTCAtivoClick(Sender: TObject);
    procedure BitBtn1Click(Sender: TObject);
  private
    ACBrMonitorINI : AnsiString;
    Inicio  : Boolean ;
    ArqSaiTXT, ArqSaiTMP, ArqEntTXT, ArqLogTXT : AnsiString ;
    Cmd : TACBrCmd ;
    NewLines : AnsiString ;
...
begin
  ...
  TrayIcon.Hint    := 'ACBrMonitor '+ Versao + sLineBreak +
                      'ACBr: '+ACBR_VERSAO ;
  ...
  Top  := Screen.Height - ClientHeight - 100;
  Left := Screen.Width  - ClientWidth  - 50;
...
begin
  ...
  ACBrTCPServer1.OnDesConecta := nil ;
  ACBrTCPServer1.Ativo        := False ;    { Desliga TCP }
end;

{------------------------------------------------------------------------------}
procedure TFrmACBrMonitor.ACBrTCPServer1Conecta(
...
begin
  ...
  Resposta('','ACBrMonitor Ver. '+ Versao + ' - ACBr: '+ACBR_VERSAO +sLineBreak +
              ...
              'Máquina: '+TCPBlockSocket.GetRemoteSinIP+sLineBreak+
              'Esperando por comandos.');

end;

{------------------------------------------------------------------------------}
procedure TFrmACBrMonitor.ACBrTCPServer1DesConecta(
   const TCPBlockSocket: TTCPBlockSocket ; Erro: Integer ; ErroDesc: String) ;
begin
  mResp.Lines.Add( 'ALERTA: Fim da Conexão com: '+
                    TCPBlockSocket.GetRemoteSinIP+
                   ' em: '+FormatDateTime('dd/mm/yy hh:nn:ss', now ) ) ;
end;

procedure TFrmACBrMonitor.ACBrTCPServer1RecebeDados(
   const TCPBlockSocket: TTCPBlockSocket ; const Recebido: AnsiString ;
   var Enviar: String) ;
 Var Comando : AnsiString ;
begin
  { Le o que foi enviado atravez da conexao TCP }
  Comando := Trim(Recebido) ;
  if Comando <> '' then
  begin
     Conexao  := TCPBlockSocket ;
     NewLines := Comando  ;
...
{------------------------------------------------------------------------------}
procedure TFrmACBrMonitor.ACBrTCPServerTCConecta(
   const TCPBlockSocket: TTCPBlockSocket ; var Enviar: String) ;
 Var Comando : AnsiString ;
begin
  ...
  begin
     ...
     Comando := Trim( TCPBlockSocket.RecvString(2000) ) ;

     mTC.Lines.Add( 'Inicio de Conexão com Terminal: ['+Comando+'] IP: ['+
                    TCPBlockSocket.GetRemoteSinIP+
                   '] em: ['+FormatDateTime('dd/mm/yy hh:nn:ss', now )+']' ) ;
  end ;
end;
...
{------------------------------------------------------------------------------}
procedure TFrmACBrMonitor.ACBrTCPServerTCDesConecta(
   const TCPBlockSocket: TTCPBlockSocket ; Erro: Integer ; ErroDesc: String) ;
begin
  mTC.Lines.Add( 'Fim de Conexão com: '+
                 TCPBlockSocket.GetRemoteSinIP+
                ' em: '+FormatDateTime('dd/mm/yy hh:nn:ss', now )  ) ;
end;

{------------------------------------------------------------------------------}
procedure TFrmACBrMonitor.ACBrTCPServerTCRecebeDados(
   const TCPBlockSocket: TTCPBlockSocket ; const Recebido: AnsiString ;
   var Enviar: String) ;
 Var Comando : AnsiString ;
begin
  ...
  begin
     mTC.Lines.Add( 'Recebido comando: '+Comando  );

     // TODO: Buscar código do Produto em um StringList e retornar DESCRICAO e Preço
     Enviar := '#DESCRICAO DO PRODUTO|100,00' ;
  end ;
end;
...
begin
  ...
  if CanClose then
     CanClose := ( MessageDlg( 'Deseja realmente encerrar o ACBrMonitor ?',
...
begin
  ...
  begin                                    //se nao existir vai para as configs
    MessageDlg( 'Bem vindo ao ACBrMonitor',
                 'Bem vindo ao ACBrMonitor'+sLineBreak+
               'Por favor configure o ACBrMonitor, '+sLineBreak+
               'informando o Método de Monitoramento, e a configuração '+sLineBreak+
               'dos Equipamentos de Automação ligados a essa máquina.'+sLineBreak+
               'IMPORTANTE: Após configurar o '+sLineBreak+
               'Método de Monitoramento o ACBrMonitor'+sLineBreak+
...
//     Application.Minimize ;
  ...
  try
     ...
     ACBrTCPServer1.Ativo := rbTCP.Checked ;
  ...
  try
     if rbTCP.Checked then
      begin
        if ACBrTCPServer1.Ativo then
        begin
           try
              Txt := 'Endereço: '+ACBrTCPServer1.TCPBlockSocket.GetRemoteSinIP;
           except
              Txt := '' ;
           end ;
           mResp.Lines.Add( Txt ) ;
           mResp.Lines.Add( 'Porta: ['+ACBrTCPServer1.Port+']' ) ;
        end ;
      end
     else
      begin
...
var Ini : TIniFile;
    ...
    Senha, ECFDeviceParams : AnsiString ;
begin
...
//     Ini.ReadString('ACBrMonitor','TXT_Saida','SAI.TXT');
     ...
     ACBrTCPServer1.Port    := edPortaTCP.Text ;
//     ACBrTCPServer1.MaxConnections := sedConexoesTCP.Value ;
...
begin
  if NewLines <> '' then
     mCmd.Lines.Add( AnsiToUtf8( NewLines ) ) ;
...
begin
  ...
  mResp.Lines.Add( AnsiToUtf8( Comando + sLineBreak + Resposta ) ) ;
     ...
     begin
       ...
       try
          try
             ...
             { Ignora Exceçoes na Tentativa de Leitura ou Deleçao }
          end ;
...
procedure TFrmACBrMonitor.PageControl1Change( Sender: TObject) ;
begin
...
begin
  ...
  mResp.Lines.Add( AnsiToUtf8( 'LCB -> '+ACBrLCB1.UltimoCodigo) ) ;
...
procedure TFrmACBrMonitor.cbxTCAtivoClick(Sender: TObject);
begin
  gbTC.Enabled := cbxTCAtivo.Checked ;
end;

procedure TFrmACBrMonitor.BitBtn1Click(Sender: TObject);
begin
  ACBrTCPServerTC.Port := edTCPort.Text ;
//  ACBrTCPServerTC.MaxConnections := seTCConexoes.Value ;

  ACBrTCPServerTC.Ativo := not ACBrTCPServerTC.Ativo ;
  mTC.Lines.Add( 'Servidor de Terminal de Consulta: '+IfThen( ACBrTCPServerTC.Ativo, 'ATIVADO', 'DESATIVADO' ) ) ;
end;
