...
uses
  ...
  ACBrUtil, ACBrLCB, ACBrDIS, ACBrGAV, ACBrGAVClass, ACBrBase, ACBrCHQ, ACBrECF,
  CmdUnit, ACBrRFD, ACBrBAL, ACBrETQ , ACBr_LCL, indylaz, IdTCPServer,
...
{------------------------------------------------------------------------------}
procedure Tdm.TCPServerConnect( AContext: TIdContext) ;
begin
  sleep(100);
  Conexao := AContext ;
  Resposta('','ACBrMonitorConsole Ver. '+ Versao + ' - ACBr: '+ACBR_VERSAO +sLineBreak +
              ...
              'Máquina: '+AContext.Connection.Socket.Binding.PeerIP+sLineBreak+
              'Esperando por comandos.');

end;

{------------------------------------------------------------------------------}
procedure Tdm.TCPServerDisconnect( AContext: TIdContext) ;
begin
...
begin
  ...
  Comando := AContext.Connection.IOHandler.ReadLn();
  Comando := Trim( Comando ) ;
  if Comando <> '' then
  begin
     Conexao := AContext ;
     ComandosAProcessar.Text := Comando  ;
...
Procedure Tdm.Resposta(Comando, Resposta : AnsiString);
begin
  if IsTCP then
  begin
     ...
     begin
        ...
        begin
           dm.Conexao.Connection.IOHandler.WriteLn(Resposta) ;
           dm.Conexao.Connection.IOHandler.Write(#3) ;
        end;
     end ;
  end ;
