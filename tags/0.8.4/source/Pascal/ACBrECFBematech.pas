...
TACBrECFBematech = class( TACBrECFClass )
 ...
 public
    ...
    Property IsMFD : Boolean read fsMFD ;  { É MFD }
    Property IsTH  : Boolean read fsTH  ;  { É Termica }
...
begin
  ...
  try
     ...
     NumVersao ;   { Inicializa fsMFD, fsNumVersao e fsTH }
  except
     Desativar ;
     raise ;
  end ;
...
begin
 { O Bug no comando 35 + 22 ainda persiste na versao 010101... Enquanto nao
  ...
  if IsMFD then   //  if NumVersao = '010000' then
     Result := fsTotalPago
  else
   begin
     BytesResp := 7 ;
...
begin
  ...
  if not IsTH then   { Se não é Termica, Imprime Linha a Linha }
...
begin
  BytesResp := 0 ;
  AguardaImpressao := True ;
  EnviaComando( #21, 5 ) ;  { Fecha o relatorio Gerencial ou Vinculado }
end;
