...
begin
  ...
  RetCMD      := Copy( RetCMD, Pos( STX, RetCMD ) +3, 12 );
  fsNumSerie  := RetCMD;
...
Function TACBrECFICash.EnviaComando( cmd : String ) : String ;
Var ErroMsg, ErroMsg2, cComando : String ;
    Erro, Erro2 : Integer;
...
begin
  ...
  try
     ...
     while  not EmLinha( 3 ) do  { Impressora está em-linha ? }
     begin
        if Retentar and
...
//          32 : ErroMsg2 := 'Operação realizada com sucesso';
     ...
     if (ErroMsg <> '') or (ErroMsg2 <> '') then begin
...
begin
   If Retorno <> ACK Then Begin
      If ( ( LeftStr( fpComandoEnviado, 2 ) = '21' ) or ( LeftStr( fpComandoEnviado, 2 ) = '26' ) or ( LeftStr( fpComandoEnviado, 2 ) = '67' ) or ( LeftStr( fpComandoEnviado, 2 ) = '2E' ) ) Then Begin
...
procedure TACBrECFICash.AbreCupom(CPF_CNPJ: String);
begin
...
Var AliquotaStr, QtdStr, ValorStr, DescontoStr, Fmt, cNumDecimais: String ;
...
begin
   ...
   case AliquotaICMS[1] of
      ...
      begin
         Aliquota := AchaICMSIndice( AliquotaICMS ) ;
         if Aliquota <> nil then
            AliquotaStr := 'T'+Format('%5.2f', [Aliquota.Aliquota])+'%'
         else
  ...
  RetCMD   := EnviaComando( '21' + Codigo + Descricao + QtdStr + Unidade + ValorStr + cNumDecimais + AliquotaStr, 7 );
  nNumItem := StrToInt( Copy( RetCMD, Pos(STX, RetCMD)+3, 3 ) );

  If DescontoPorc >0 Then
     EnviaComando( '261' + DescontoSTR + '000000000000' + IntToStrZero( nNumItem, 3 ), 4  );
end;
...
procedure TACBrECFICash.PreparaTEF;
begin
