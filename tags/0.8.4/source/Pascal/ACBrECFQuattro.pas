...
Function TACBrECFQuattro.EnviaComando( cmd : String ) : String ;
Var ErroMsg : String ;
    Erro : Integer ;
    LeituraMF : Boolean ;
    Verificar : Boolean ;
  ...
  try
     ...
     if ErroMsg <> '' then
      begin
        ...
        { Quattro precisa de uma pequena pausa entre um comando e outro }
        Sleep(20) ;
...
function TACBrECFQuattro.VerificaFimLeitura(Retorno:String) : Boolean;
begin
...
function TACBrECFQuattro.VerificaFimImpressao: Boolean;
Var RetCmd : String ;
    I : Integer ;
begin
  ...
  begin
   begin
     ...
     if LeftStr(RetCmd, 4) = '.+03' then
        fsNumSerie := Trim(Copy(RetCmd,5,6)) ;
   end ;
  end ;
...
begin
  ...
  begin
     ...
     if P > 0 then
     begin
        SubTot := StrToIntDef(copy(RetCmd,19,12),0) / 100;
        SubTot := RoundTo(SubTot, -2) ;
        Falta  := StrToIntDef(copy(RetCmd,100,12),0) / 100 ;
...
begin
  ...
  if pos('!',RetCmd) > 0 then
     Result := RoundTo(StrToIntDef(copy(RetCmd,19,12),0) / 100, -2) ;
end;
...
procedure TACBrECFQuattro.AbreCupom(CPF_CNPJ: String);
begin
...
Var AliquotaStr, QtdStr, ValorStr, Descr2 : String ;
...
procedure TACBrECFQuattro.PreparaTEF;
begin
...
begin
  ...
  if fsArqFormato_ChequeTXT = '' then
     ArqTemp := ExtractFilePath( Application.ExeName )+'Formato_cheque.txt'
  else
