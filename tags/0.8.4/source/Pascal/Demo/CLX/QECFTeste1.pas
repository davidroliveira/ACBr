...
uses ACBrECF, ACBrBase, ACBrECFClass,  
...
uses ACBrUtil, ACBrECFBematech, QVendeItem, QEfetuaPagamento,
     QRelatorio, DateUtils, TypInfo, Math;
...
begin
  ...
  if VerificaEstado then
     try
        ...
        { GetEnumName(TypeInfo(TACBrECFEstado), interger( ACBrECF1.Estado ) ) ;}
     except
     end ;
end;
...
begin
  if ACBrECF1.Modelo <> ecfDataRegis then
...
begin
  ...
  if MessageDlg('O Comprovante Nao Fiscal: ['+cDescricao+'] '+
                IfThen(ACBrECF1.Modelo in [ecfDaruma,ecfSchalter,ecfSweda,ecfQuattro],
                       ' do Tipo: ['+cTipo+'] ','') +
