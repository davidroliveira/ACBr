...
{       if AUT = '5' then
     ...
     if ErroMsg <> '' then
      begin
        ...
        { Sweda precisa de uma pequena pausa entre um comando e outro }
        if fsVersaoSweda >= swdST then Sleep(20) else Sleep(100) ;
...
Var Espera : Integer ;
begin
  Espera := IfThen( fsVersaoSweda >= swdST, 10,  20) + (ReducaoFinal - ReducaoInicial) ;
  AguardaImpressao := True ;
...
Var Espera : Integer ;
begin
  Espera := IfThen( fsVersaoSweda >= swdST, 10,  20) + DaysBetween(DataInicial,DataFinal) ;
  AguardaImpressao := True ;
